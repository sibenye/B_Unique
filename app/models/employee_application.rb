class EmployeeApplication < ActiveRecord::Base
  ...
  attr_accessible :ssn, :dl_number, :dl_issue, :applying_for, :employment_type, :period, :committment, :employer_name, :employer_number, :employer_address, :employer_city, :employer_state, :employer_zipcode, :employer_country
  ...
  if row != nil and row != ""
    ...
    if row.accepted != nil and row.accepted != ""
      if row.accepted == true
        result['message']='You have been offered the position'
      else
